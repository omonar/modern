    ...
    function Monitor( index, id, connKey ) {
      ...
      this.getStreamCmdResponse = function( respObj, respText ) {
        ...
        if ( respObj.result == 'Ok' ) {
          ...
          this.setStateClass( $moo('monitor'+this.index), stateClass );
          ...
          if ( newAlarm ) {
            if ( false && SOUND_ON_ALARM ) {
              ...
              $moo('alarmSound').removeClass( 'hidden' );
            }
          ...
          if ( false && SOUND_ON_ALARM ) {
            if ( oldAlarm ) {
              // Disable alarm sound
              $moo('alarmSound').addClass( 'hidden' );
            }
          }
        }
        else {
      ...
      <div style="display:none;">
        <div id="viewcontrol" >
          ...
          <table width="100%"><tr valign="top"><td>
            ...
            <h2>Columns</h2><hr>
            <div>
              <div><button class="btn btn-default btngridsize" type="button" onclick="loadcameragrid(this,2);" number="2">2 Columns</button></div>
              <div><button class="btn btn-default btngridsize" type="button" onclick="loadcameragrid(this,3);" number="3" DISABLED>3 Columns</button></div>
              <div><button class="btn btn-default btngridsize" type="button" onclick="loadcameragrid(this,4);" number="4">4 Columns</button></div>
            </div>
            ...
            <div>
              ...
              foreach( $displayMonitors as $monitor ) {
                echo '<div><button class="btn btn-default btnmonitor" type="button" onclick="loadcameramonitor(this,'.$monitor['Id'].');return false;" number="'.$monitor['Id'].'">'.$monitor['Name'].'</button></div>';
              }
              ?>
            </div>  
